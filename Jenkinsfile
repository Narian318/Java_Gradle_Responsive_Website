            ...
            steps{
                script{
                    withCredentials([string(credentialsId: 'docker_nexus_pass', variable: 'docker_nexus_passwd')]) {
                                ...
                                sh ''' 
                                docker build -t 34.230.58.238:8083/springapp:$BUILD_ID .
                                docker login -u admin -p sonarqube 34.230.58.238:8083 
                                docker push  34.230.58.238:8083/springapp:$BUILD_ID
                                docker rmi 34.230.58.238:8083/springapp:$BUILD_ID
                            '''
                        }
                    } 
                }
